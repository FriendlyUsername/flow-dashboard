...
export const ChangeLogger = ({ limit = 20 }: ChangeLoggerProps) => {
  ...
  useEffect(() => {
    // Store the original onNodesChange handler
    const originalOnNodesChange = store.getState().onNodesChange;

    // Create a new handler that calls both the original and our logger
    const combinedHandler: OnNodesChange = (changes) => {
      // Call the original handler first (if it exists)
      if (originalOnNodesChange) {
        originalOnNodesChange(changes);
      }
      // Then call our logger
      handleNodeChanges(changes);
    };

    store.setState({ onNodesChange: combinedHandler });

    return () => {
      // Restore the original handler when unmounting
      store.setState({ onNodesChange: originalOnNodesChange });
    };
